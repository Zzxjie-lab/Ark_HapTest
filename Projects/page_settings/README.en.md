...
The **Navigation** component is used to implement single-column or dual-column display. It consists of two areas: **Navbar** (main page) and **Content** (subpage). The **Navigation** component supports the **Stack**, **Split**, and **Auto** modes. For details about the source code, see [products/default/src/main/ets/pages/Index.ets](products/default/src/main/ets/pages/Index.ets).
...
3. Auto mode: The **Navigation** component can automatically select a proper mode based on the application window size. If the window width is less than 520 vp, the **Stack** mode is applied. If the window width is greater than or equal to 520 vp, the **Split** mode is applied. When the window size changes, the **Navigation** component automatically switches between the Stack mode and the Split mode. 

#### Redirect or Re-render upon a Tap
The **Navigation** component is usually used together with the **NavPathStack** object and **NavDestination** component.
* The **NavPathStack** object controls page navigation within the **Navigation** component by pushing the specified **NavDestination** page information onto the stack through Push-related APIs. For details about the source code, see [features/settingitems/src/main/ets/wlan/WlanSettingItem.ets](features/settingitems/src/main/ets/wlan/WlanSettingItem.ets).
* The **NavDestination** component is responsible for refreshing the display within the **Content** area of the **Navigation** component.

#### Multi-level jump
The **Navigation** component can automatically switch between the single- and dual-column mode. In addition, the component can add the back button and respond to the back button event called by the system based on the current state. For details about the source code, see [features/settingitems/src/main/ets/wlan/WlanMoreSetting.ets](features/settingitems/src/main/ets/wlan/WlanMoreSetting.ets). 
